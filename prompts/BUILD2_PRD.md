...
src/agent_team/
  ...
  mcp_client.py             NEW  ~200 lines  MCP session management
...
- [ ] REQ-012: `generate_teammate_idle_hook()` must return an agent-type hook that calls TaskList, exits code 2 if pending unblocked tasks exist (preventing idle), allows idle if all tasks are completed or blocked — timeout 30 seconds (review_cycles: 0)
...
- [ ] WIRE-001: Wire `create_execution_backend()` call into `cli.py` mode selection block (line ~4200), before the existing milestone/standard branch, gated on `config.agent_teams.enabled` (review_cycles: 0)
...
- [ ] TEST-007: Test `generate_teammate_idle_hook()` produces agent-type hook dict with timeout=30 (review_cycles: 0)
...
- Status: PENDING
- Dependencies: none
- Description: Create typed MCP client for Build 1's Contract Engine (6 tools), MCP session management utilities, ServiceContractRegistry for local+MCP contract management, and MCP server configuration. Every MCP call has try/except returning safe defaults on failure, enabling fallback to existing static scans. Note: mcp_client.py is created in this milestone (not M1), so M2 has no dependencies.

### Functional Requirements

- [ ] REQ-017: Create `src/agent_team/contract_client.py` with `ContractEngineClient` class wrapping all 6 Contract Engine MCP tools (review_cycles: 0)
...
| SVC-009 | CodebaseIntelligenceClient.find_dependencies(file_path) | find_dependencies | { file_path: string } | { imports: array, imported_by: array, transitive_deps: array } |
| SVC-010 | CodebaseIntelligenceClient.search_semantic(query, n_results) | search_semantic | { query: string, n_results: number } | array |
...
- [ ] SVC-009: CodebaseIntelligenceClient.find_dependencies(file_path) -> MCP find_dependencies { file_path: string } -> { imports: array, imported_by: array, transitive_deps: array } (review_cycles: 0)
- [ ] SVC-010: CodebaseIntelligenceClient.search_semantic(query, n_results) -> MCP search_semantic { query: string, n_results: number } -> array (review_cycles: 0)
...
- [ ] REQ-053: `write_teammate_claude_md(project_dir, role, service_name, config, contracts, mcp_servers) -> Path` must write to `{project_dir}/.claude/CLAUDE.md` creating the `.claude/` directory if needed, returning the path written (review_cycles: 0)
...
- [ ] REQ-055: In `cli.py` at line ~4150 (before mode selection block, after codebase map generation), when `config.contract_engine.enabled`, create `ServiceContractRegistry`, call `load_from_mcp()`, then `save_local_cache()` — falling back to `load_from_local()` only on MCP failure. Wrap in try/except with logger.warning on failure (review_cycles: 0)
...
- [ ] TEST-044: Test `generate_claude_md()` truncates contracts list at 20 with "... and N more" suffix (review_cycles: 0)
...
- [ ] TECH-044: In `apply_depth_quality_gating()` (the actual function name in config.py), set depth gating for new features: quick = all contract scans off + contract_engine off + codebase_intelligence off + agent_teams off; standard = contract_engine enabled (validation_on_build=True, test_generation=False) + codebase_intelligence enabled (replace_static_map=False, register_artifacts=False — queries via find_definition/search_semantic only, codebase map uses static analysis) + CONTRACT 001-002 on; thorough = full contract_engine (test_generation=True) + full codebase_intelligence (replace_static_map=True, register_artifacts=True) + all 4 CONTRACT scans + agent_teams enabled (if env set); exhaustive = same as thorough with no differences. Note: depth-based defaults can be overridden by explicit YAML config — user overrides tracked via `set[str]` from `_dict_to_config()` take precedence over depth defaults (review_cycles: 0)
