...
- **Docker Orchestration:** Direct `docker compose` subprocess calls via asyncio.create_subprocess_exec
...
- **Secret Scanning:** Custom regex patterns (SEC-SECRET-001..012)
...
Key constraint: base image MUST be Debian-based (see TECH-031).
...
- **Parallel with: M3** (both depend only on M1)
...
- [ ] REQ-021: Create `src/integrator/pact_manager.py` with `PactManager` class: `__init__(self, pact_dir: Path)`, `load_pacts(self) -> dict[str, list[Path]]` scanning pact_dir for *.json Pact files grouped by provider name, `async verify_provider(self, provider_name: str, provider_url: str, pact_files: list[Path]) -> list[ContractViolation]` using `from pact.v3.verifier import Verifier; verifier = Verifier(provider_name)`, adding transport via `verifier.add_transport(url=provider_url)`, adding each pact file via `verifier.add_source(str(pf))`, setting state handler via `verifier.state_handler(handler_function, teardown=True)`, then executing `try: result = verifier.verify(); except Exception as e:` — `verify()` returns `Self` (the Verifier instance) on success for method chaining; on failure it raises an exception. IMPORTANT: `Verifier(name)` IS the correct v3 constructor — `Verifier()` with no args is WRONG. `set_info()` does NOT exist in v3 — use `add_transport(url=url)`. `set_state_handler()` does NOT exist — use `state_handler()`. `set_state()` does NOT exist. All Pact Verifier methods are synchronous — the async `verify_provider()` must wrap `verifier.verify()` with `await asyncio.to_thread(verifier.verify)` to avoid blocking the event loop. Convert failures to ContractViolation with code "PACT-001" for interaction failures and "PACT-002" for state setup failures (review_cycles: 0)
...
- [ ] TECH-013: Pact provider verification must use `from pact.v3.verifier import Verifier`. Constructor is `Verifier(provider_name)` — `Verifier(name)` IS the correct v3 constructor; `Verifier()` with no args is WRONG. Chain `.add_transport(url=url)` to set the provider URL. The `.verify()` method returns `Self` (the Verifier instance) on success for method chaining. On failure it raises an exception. There is NO `set_info()` method, NO `pact.error.PactVerificationError` class, and NO `set_state()` method — use `state_handler()` (not `set_state_handler()`). All Pact Verifier calls are synchronous and must be wrapped in `asyncio.to_thread()` when called from async code (review_cycles: 0)
...
- [ ] TECH-014a: Schemathesis and Pact libraries are synchronous blocking libraries. All calls to `schemathesis.openapi.from_url()`, `case.call()`, `case.validate_response()`, `verifier.verify()`, and `verifier.state_handler()` MUST be wrapped with `await asyncio.to_thread(...)` when called from async methods to avoid blocking the event loop. Example: `schema = await asyncio.to_thread(schemathesis.openapi.from_url, openapi_url, base_url=base_url)` (review_cycles: 0)
...
| SVC-002 | PactManager.verify_provider(provider_name, provider_url, pact_files) | pact.v3.verifier.Verifier(name).add_transport(url=url).add_source(file).verify() -> Self / raises on failure | { provider_name: string, provider_url: string, pact_files: list[string] } | list[ContractViolation] |
...
- [ ] SVC-002: PactManager.verify_provider(provider_name, provider_url, pact_files) -> pact.v3.verifier.Verifier(name).add_transport(url=url).add_source(file).verify() returns Self / raises on failure { provider_name: string, provider_url: string, pact_files: list[string] } -> list[ContractViolation] (review_cycles: 0)
...
- [ ] REQ-039: Create `src/quality_gate/adversarial_patterns.py` with `AdversarialScanner` class (purely static, NO MCP dependency): `__init__(self, project_root: Path)`, `detect_dead_events(self) -> list[ScanViolation]` finding ADV-001 using compiled regex: publisher pattern `_RE_EVENT_PUBLISH = re.compile(r"(?:publish|emit|dispatch|send_event|fire)\s*\(\s*['\"](\w+)['\"]")`, consumer pattern `_RE_EVENT_SUBSCRIBE = re.compile(r"(?:subscribe|on|addEventListener|consume|handle|listen)\s*\(\s*['\"](\w+)['\"]")`. Scan all .py/.ts/.js files (excluding node_modules, .venv, __pycache__, dist, build). Use `pathlib.Path.rglob()` for file discovery with `EXCLUDE_DIRS = {"node_modules", ".venv", "venv", "__pycache__", "dist", "build", ".git"}` filtered in the iteration loop (check `if any(part in EXCLUDE_DIRS for part in path.parts): continue`). Do NOT use `os.walk()`. Collect published event names and consumed event names. Report ADV-001 for each event name in publishers but not consumers (review_cycles: 0)
...
**File Walking Mechanism:** Use `pathlib.Path.rglob()` for file discovery: `EXCLUDE_DIRS = {'node_modules', '.venv', '__pycache__', 'dist', 'build', '.git', '.tox', 'egg-info'}`. For each extension (.py, .ts, .js, .jsx, .tsx), call `project_root.rglob(f'*{ext}')` and filter: `if not any(excluded in p.parts for excluded in EXCLUDE_DIRS)`. Do NOT use `os.walk()` — pathlib is more reliable for cross-platform path handling.
...
|--------|---------------|----------------------|-------------|--------------|
| SVC-005 | run_architect_phase(prd_path) | MCP stdio: python -m src.architect.mcp_server -> decompose (fallback: subprocess + JSON) | { prd_text: string } | { service_map: ServiceMap (see Build 1 REQ-002 for field schema), domain_model: DomainModel (see Build 1 REQ-002 for field schema), contract_stubs: list } |
...
| SVC-009 | run_parallel_builders(builder_configs) | subprocess: python -m agent_team --cwd {dir} --depth {depth} | { service_id: string, output_dir: string, config: dict } | BuilderResult (see M1 REQ-002 for field schema) { success: boolean, cost: number, test_passed: number, test_total: number } |
| SVC-010 | run_quality_gate(integration_report, builder_results) | Internal: QualityGateEngine.run_all_layers() | { builder_results: list[BuilderResult] (see M1 REQ-002 for field schema), integration_report: IntegrationReport } | QualityGateReport { overall_verdict: string, layers: dict, total_violations: number } |
...
- Description: Implement the Typer CLI with 8 CLI commands: init, plan, build, integrate, verify, run (6 pipeline commands) + status, resume (2 utility commands), Rich terminal UI with progress bars and live dashboard, and the YAML configuration template for the entire super team.
...
- [ ] TECH-032: Add the following to pyproject.toml in the project root: `[tool.pytest.ini_options]` with `asyncio_mode = 'auto'` and `filterwarnings = ['ignore::DeprecationWarning']`. This enables automatic async test detection — any test function defined as `async def` is automatically treated as an asyncio test without requiring the `@pytest.mark.asyncio` decorator (review_cycles: 0)
- [ ] TECH-031: All Docker images for Build 3 services and test runners SHOULD use a Debian-based Python image (e.g., python:3.12-slim-bookworm). Alpine/musl support for pact_ffi is experimental and may cause runtime issues, particularly on ARM64 architectures (review_cycles: 0)
...
> **Note:** These SEC-xxx IDs (SEC-001..004) are infrastructure security requirements for operational deployment, distinct from the JWT vulnerability scan codes SEC-001..006 defined within REQ-034's SecurityScanner patterns.
...
- Security: Custom regex patterns (SEC-SECRET-001..012) + PyJWT 2.8+
