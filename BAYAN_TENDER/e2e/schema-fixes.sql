-- BAYAN Tender E2E Schema Fixes
-- Fixes migration gaps between EF Core entity definitions and actual DB schema

-- tender_bidders: invited_at/invited_by are orphan NOT NULL columns from InitialCreate
-- Entity uses invitation_sent_at instead. Make them nullable + set defaults.
DO $$ BEGIN
...
-- bidder_refresh_tokens: table missing from EF migrations (needed for portal login)
...
-- evaluation_panels: add missing completed_at, make extra cols nullable
...
-- Make evaluation_panels extra columns nullable (EF doesn't know about them)
DO $$ BEGIN
...
-- bidders: add activation fields (for bidder account activation flow)
...
-- clarification_bulletins: add missing columns + fix constraints
...
-- title column exists in migration but not in entity — make nullable
DO $$ BEGIN
...
-- audit_logs: widen action + entity_type columns (URLs can exceed 100 chars)
DO $$ BEGIN
...
-- audit_logs: drop FK to users (bidder IDs are valid audit subjects too)
...
-- vendor_pricing_snapshots: add missing columns required by EF entity
...
-- Add FK constraints if possible (may fail if references don't exist yet, that's OK)
DO $$ BEGIN
...
-- vendor_pricing_snapshots: created_by NOT NULL but entity doesn't map it — make nullable
DO $$ BEGIN
...
-- vendor_item_rates: add missing columns required by EF entity
...
-- Fix boq_item_id nullable (entity config does NOT require it)
DO $$ BEGIN
  ...
  END IF;
END $$;
-- Rename currency to normalized_currency if old name exists
DO $$ BEGIN
...
-- vendor_item_rates: rank column has no DEFAULT but entity doesn't map it
ALTER TABLE vendor_item_rates ALTER COLUMN rank SET DEFAULT 0;
-- vendor_item_rates: bid_submission_id/bidder_id/tender_id are NOT NULL in migration but entity doesn't set them
DO $$ BEGIN
...
-- addendum_acknowledgments: add missing columns + fix nullable
